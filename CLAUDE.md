...
The operator-facing material lives in `README.md`: per-provider provisioning (GCP, EC2, Vultr), the port list, the feature-code table, the post-install checklist, and a step table mirroring `install.sh`'s `STEPS` array — keep that table in sync when adding a step, and do not restate any of it here. `portal/api/README.md` and `portal/web/README.md` are untouched `bun init` / Vite-template scaffolding and carry no project information.
...
- A `validate` hook on `crudRoutes` gets `existing`, the pre-update row, and a paired-field check **must** use it. A PATCH carries only the fields being changed, so validating `body` alone sees half a pair as absent and skips the check entirely — that is how a cross-tenant id gets written, by sending it without its type.
...
- **The schema reaches Postgres through `drizzle-kit push --force`, not through the migration files.** `41-api-deploy.sh` diffs `src/db/schema.ts` against the live database; `drizzle-kit migrate` is never run, so `drizzle/0000_init.sql` and its journal are `db:generate` output that nothing applies. `schema.ts` is the only source of truth — hand-editing the SQL does nothing. Two consequences: `--force` skips confirmation, so a rename or a narrowed column applies destructively to an existing install; and `drizzle.config.ts` must keep listing `schema.ts` *and* `auth-schema.ts` explicitly, because drizzle-kit parses files rather than following re-exports and would otherwise emit a schema with no auth tables.
- **Plaintext SIP secrets never leave the API.** The `*Enc` columns (SIP passwords, voicemail PINs, trunk credentials) are AES-256-GCM in `src/lib/crypto.ts`, stored as `iv:tag:ciphertext` base64; `services/kamailio-sync.ts` decrypts, computes HA1/HA1b and writes only the digest into Kamailio's `subscriber` table. That projection is **one-way** on purpose — our schema wins on disagreement and a rebuild fixes it — which is why it uses its own pool and its own credentials.
...
- **Bind xml_curl only to sections the API actually serves — `directory|dialplan`.** `configuration` was bound as well, with a comment claiming the API served it; `routes/fs-xml.ts` answers every configuration request with "not found", so it only ever cost an HTTP round trip per section at module load and on `reloadxml`. During an install it produced exactly 12 `[ERR] mod_xml_curl.c:319 Received HTTP error 0` lines, because FreeSWITCH starts at step 30 and the API does not exist until step 41. Reproduced both ways in the container: 12 errors with the binding, 0 without, API stopped in both cases.
- Related, and the reason it went unnoticed: **99-postflight scopes its `[ERR]`/`[CRIT]` count to the current FreeSWITCH run**, because the log appends across restarts and an unscoped count reports errors from a life that has already been restarted away. Compare `substr($0,1,19)` lexicographically against `systemctl show freeswitch -p ActiveEnterTimestamp` rendered as `%Y-%m-%d %H:%M:%S` — mawk is Debian's awk and has no `mktime()`. Require the timestamp prefix in the same pattern: untimestamped console output would otherwise land inside every window, since letters sort above digits.
- **Every polymorphic destination resolves in one place: `voip.go` (`resources/lua/voip.lua`).** An inbound route, an IVR option, a ring-group failover and both branches of a time condition all point at "an extension, or a ring group, or an IVR, or…", so a new destination type is added there and nowhere else. `voip.go` also increments and enforces `depth` against routing loops (an IVR option pointing at a time condition pointing back); every call-time script therefore takes `[depth]` as `argv[3]` and forwards it. A script that drops it re-opens the loop `MAX_DEPTH` exists to close.
...
# (`bun run routes` regenerates it alone, for when it is stale mid-edit).
cd portal/api && bunx tsc -p tsconfig.json --noEmit   # see below; `bun run typecheck` exits 127
cd portal/api && bun run format                       # prettier; there is no lint here
cd portal/web && bun run typecheck && bun run lint && bun run build
```

**`portal/api`'s `typecheck` script is broken and has never run:** `typescript` is not among its dependencies (nothing in `bun.lock`, no `node_modules/typescript`), so `tsc --noEmit` exits 127 with `command not found` unless the machine happens to have a global `tsc`. Use `bunx tsc -p tsconfig.json --noEmit`, which fetches it and passes clean, or add the devDependency. `portal/web` pins `typescript` properly, so its script works.

Two other portal commands need setting up before they work. Any `drizzle-kit` command (`db:generate`, `db:push`, `db:studio`) reads `DATABASE_URL` and `drizzle.config.ts` throws without it — the installer writes one into `/opt/voip-api/.env`, so local work needs a `portal/api/.env`. `typecheck` does not, which is why the failure only shows up when you reach for drizzle. And `src/db/auth-schema.ts` is **committed, not generated at deploy**: change a better-auth plugin or field and re-run `bun run auth:generate` by hand.
...
- The xml_curl binding and the scoped postflight count were verified 2026-07-30 in the arm64 container, reproduced both ways with the API stopped: 12 `mod_xml_curl.c:319 Received HTTP error 0` lines with `configuration` bound, 0 after narrowing the binding to `directory|dialplan`.
